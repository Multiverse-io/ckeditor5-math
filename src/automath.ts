...
export default class AutoMath extends Plugin {
	...
	public init(): void {
		...
		this.listenTo( editor.plugins.get( ClipboardPipeline ), 'inputTransformation', () => {
			...
			const leftModelLivePosition = ModelLivePosition.fromPosition( firstRange.start );
			leftModelLivePosition.stickiness = 'toPrevious';

			const rightModelLivePosition = ModelLivePosition.fromPosition( firstRange.end );
			rightModelLivePosition.stickiness = 'toNext';

			modelDocument.once( 'change:data', () => {
				this._mathBetweenPositions(
					leftModelLivePosition,
					rightModelLivePosition
				);

				leftModelLivePosition.detach();
				rightModelLivePosition.detach();
			},
			{ priority: 'high' }
			);
		}
